function [ panel_pos, fig, all_ax, h ] = panelFigureSetup2( panel_x, panel_y, labels, figure_width, ...
...
set(fig, 'units', get(gcf, 'paperunits'), 'position', [1,1.5, get(gcf, 'papersize')]);
...
%labels=['ACEBDF'];
totalPanels = panel_x * panel_y;
labelLength = length(labels);
if(totalPanels<labelLength)
    labels = labels(1:totalPanels);
end
labelLength = length(labels);
while(totalPanels>labelLength)
    labels = strcat(labels,'_');
    labelLength = length(labels);    
end
...
for i = 1 : panel_x * panel_y
    ...
    h(i) = text(panel_pos(i,1)-panel_spacing_x/4, panel_pos(i,2) + panel_pos(i,4) + panel_spacing_y/8, labels(i), 'fontweight', 'bold'); %%
